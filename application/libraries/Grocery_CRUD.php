...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function _print_webpage($data)
	{
		...
		#print-table{ color: #000; background: #fff; font-family: Verdana,Tahoma,Helvetica,sans-serif; font-size: 10px;}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	 ...
	 * @param $url_callback
	 */
	public function add_action( $label, $image_url = '', $link_url = '', $css_class = '', $url_callback = null, $btn_class = 'btn-default', $not_hide = false, $literal = false)
	{
		...
		$this->actions[$unique_id]  = (object)array(
			...
			'url_has_http'	=> $literal || (substr($link_url,0,7) == 'http://') || (substr($link_url,0,8) == 'https://') ? true : false,
                        'not_hide'      => $not_hide,
                        'btn_class'     => $btn_class
		);
